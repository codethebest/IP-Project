...
public class ItemsDAO
{
	...
	public void updateItem(Items item)
	{
		try
		{
			SqlConnection myConn = new SqlConnection(connString);
			myConn.Open();
			string updateString = "UPDATE Items SET customerID = @customerID, barcode = @barcode, itemType = @itemType, description = @description, model = @model, issue = @issue, assignedToTechnician = @assignedToTechnician, dateBookedIn = @dateBookedIn WHERE itemID = @itemID";
